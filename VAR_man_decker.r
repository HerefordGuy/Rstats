...
######################################################################################
# edited by Jared E. Decker
# plot variances rather than p-values. 
# put cutoff lines as NULL values. Changed if statements for cutoff lines to !is.null.
...
manhattan = function(dataframe, colors=c("gray10", "gray50"), ymax="max", cex.x.axis=1, limitchromosomes=1:30, suggestiveline=NULL, genomewideline=NULL, annotate=NULL, ...) {
    ...
    if (numchroms==1) {
        with(d, plot(pos, V, ylim=c(0,ymax), ylab="Variance", xlab=paste("Chromosome",unique(d$CHR),"position"), ...))
    }	else {
        with(d, plot(pos, V, ylim=c(0,ymax), ylab="Variance", xlab="Chromosome", xaxt="n", type="n", ...))
        axis(1, at=ticks, lab=unique(d$CHR), cex.axis=cex.x.axis)
        icol=1
        for (i in unique(d$CHR)) {
            with(d[d$CHR==i, ],points(pos, V, col=colors[icol], ...))
            icol=icol+1
    	}
    }
