...
plot_tree_internal = function(d, e, o = NA, cex = 1, disp = 0.005, plus = 0.005, arrow = 0.05, ybar = 0.01, scale = T, mbar = F, mse = 0.01, plotmig = T, plotnames = T, xmin = 0){
	...
	lines( c(mse*10, mse*10), c(ybar- 0.01, ybar+ 0.01))
        ...
        if (mbar){
                ...
                yma = ybar+0.35
                l = 0.2
                w = l/200 ###changed to 200
                xma = max(d$x/20)
                ### changed 100 to 200, changed 99 to 199
                rect( rep(0, 200), ymi+(0:199)*w, rep(xma, 200), ymi+(1:200)*w, col = mcols, border = mcols)
