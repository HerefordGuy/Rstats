...
# Changed Number of chromosomes to 39 for dog.
...
manhattan = function(dataframe, colors=c("gray10", "gray50"), ymax="max", cex.x.axis=1, limitchromosomes=1:39, suggestiveline=FALSE, genomewideline=FALSE, annotate=NULL, ...) {

    d=dataframe
    if (!("CHR" %in% names(d) & "BP" %in% names(d) & "Q" %in% names(d))) stop("Make sure your data frame contains columns CHR, BP, and Q")
    
    if (any(limitchromosomes)) d=d[d$CHR %in% limitchromosomes, ]
    d=subset(na.omit(d[order(d$CHR, d$BP), ]), (Q>0 & Q<=1)) # remove na's, sort, and keep only 0<Q<=1
    d$logq = -log10(d$Q)
    d$pos=NA
    ticks=NULL
    lastbase=0
    colors <- rep(colors,max(d$CHR))[1:max(d$CHR)]
    if (ymax=="max") ymax<-ceiling(max(d$logq))
    if (ymax<1) ymax<-1
    ...
    if (numchroms==1) {
        with(d, plot(pos, logq, ylim=c(0,ymax), ylab=expression(-"log"[10](italic(q))), xlab=paste("Chromosome",unique(d$CHR),"position"), ...))
    }	else {
        with(d, plot(pos, logq, ylim=c(0,ymax), ylab=expression(-"log"[10](italic(q))), xlab="Chromosome", xaxt="n", type="n", ...))
        axis(1, at=ticks, lab=unique(d$CHR), cex.axis=cex.x.axis)
        icol=1
        for (i in unique(d$CHR)) {
            with(d[d$CHR==i, ],points(pos, logq, col=colors[icol], ...))
            icol=icol+1
    	}
    }
    ...
    if (!is.null(annotate)) {
        ...
        with(d.annotate, points(pos, logq, col="green3", ...)) 
    }
...
qq = function(pvector, ...) {
    if (!is.numeric(pvector)) stop("D'oh! Q value vector is not numeric.")
...
library(ggplot2)
	...
	if ("CHR" %in% names(d) & "BP" %in% names(d) & "P" %in% names(d) ) {
		...
		if (max.y=="max") maxy=ceiling(max(d$logq)) else maxy=max.y
		if (maxy<8) maxy=8
		if (annotate) d.annotate=d[as.numeric(substr(d$SNP,3,100)) %in% SNPlist, ]
		if (numchroms==1) {
			plot=qplot(pos,logq,data=d,ylab=expression(-log[10](italic(p))), xlab=paste("Chromosome",unique(d$CHR),"position"))
		}	else {
			plot=qplot(pos,logq,data=d, ylab=expression(-log[10](italic(p))) , colour=factor(CHR))
