...
manhattan = function(dataframe, colors=c("gray10", "gray50"), ymax="max", cex.x.axis=1, limitchromosomes=1:30, annotate=NULL, ...) {

    d=dataframe
    if (!("CHR" %in% names(d) & "BP" %in% names(d) & "ASE" %in% names(d))) stop("Make sure your data frame contains columns CHR, BP, and ASE")
    ...
    if (numchroms==1) {
        with(d, plot(pos, abASE, ylim=c(0,ymax), ylab="Absolute Value of Allele Substitution Effect", xlab=paste("Chromosome",unique(d$CHR),"position"), ...))
    }	else {
        with(d, plot(pos, abASE, ylim=c(0,ymax), ylab="Absolute Value of Allele Substitution Effect", xlab="Chromosome", xaxt="n", type="n", ...))
        axis(1, at=ticks, lab=unique(d$CHR), cex.axis=cex.x.axis)
        icol=1
        for (i in unique(d$CHR)) {
            with(d[d$CHR==i, ],points(pos, abASE, col=colors[icol], ...))
            icol=icol+1
    	}
    }
    ...
    if (!is.null(annotate)) {
        ...
        with(d.annotate, points(pos, abASE, col="green3", ...)) 
    }
