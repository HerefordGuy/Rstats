...
manhattan = function(dataframe, colors=c("gray10", "gray50"), ymax="max", cex.x.axis=1, limitchromosomes=1:30, suggestiveline=FALSE, genomewideline=FALSE, annotate=NULL, ...) {
    ...
    if (numchroms==1) {
        with(d, plot(pos, V, ylim=c(0,ymax), ylab=expression(abs("Alpha")/sqrt("Variance")), xlab=paste("Chromosome",unique(d$CHR),"position"), ...))
    }	else {
